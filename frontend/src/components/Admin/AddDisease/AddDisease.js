...
class AddDisease extends Component {
  ...
  componentDidMount = () => {
    axios.get(`${backendServer}/diseases`).then((response) => {
      this.setState({ allDiseases: response.data });
    });
    axios.get(`${backendServer}/disease/1`).then((response) => {
      this.setState({ disease1: response.data });
    });
  };
